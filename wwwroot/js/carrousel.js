...
(function ($) {
    ...
    $.carrouselmain = function (options, element) {
        this.$el = $(element);
        this._init(options);
    };

    $.carrouselmain.defaults = {
        current: 0,	// index of current item
        autoplay: true,// slideshow on / off
        interval: 3000  // time between transitions
    };

    $.carrouselmain.prototype = {
        // 初始化
        _init: function (options) {
            this.options = $.extend(true, {}, $.carrouselmain.defaults, options);
        ...
        _layout: function () {
            ...
            this.$currentItm.off('click.carrouselmain');
        ...
        _loadEvents: function () {
            var _self = this;
            this.$navPrev.on('click.carrouselmain', function () {
                _self._navigate('prev');
                return false;
            });

            this.$navNext.on('click.carrouselmain', function () {
                _self._navigate('next');
                return false;
            });

            this.$wrapper.on('webkitTransitionEnd.carrouselmain transitionend.carrouselmain OTransitionEnd.carrouselmain', function () {
                ...
                switch (dir) {
                    case 'next' :
                        ...
                        if (this.itemsCount > 5) {
                            ...
                            this.$prevItm.off('click.carrouselmain');
                        }
                        ...
                        $(nextEle).off('click.carrouselmain');
                    ...
                    case 'prev' :
                        ...
                        if (this.itemsCount > 5) {
                            ...
                            this.$nextItm.off('click.carrouselmain');
                        }
                        ...
                        $(prevEle).off('click.carrouselmain');
    ...
    $.fn.carrouselmain = function (options) {
        if (typeof options === 'object') {
            this.each(function () {
                var instance = $.data(this, 'carrouselmain');
                if (!instance) {
                    $.data(this, 'carrouselmain', new $.carrouselmain(options, this));
                }
            });
        }
